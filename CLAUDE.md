...
1. Read `HANDOFF.md` in full. CLAUDE.md is auto-loaded into context; HANDOFF.md is NOT. Skipping it means starting blind. This has been violated in multiple consecutive sessions.
...
Session analytics (/insights, 12 sessions to 2026-06-10) show the same failures cause nearly all friction: Wrong Approach ×11, Buggy Code ×9, Misunderstood Request ×7. Every one maps to a row below. Check this table before every response that touches code.
...
- `dotnet build Steaming.WinUI/Steaming.WinUI.csproj -c Release` AND `-c Debug` for C# WinUI (the only shipping app).
...
### 23. Use Sonnet agents for quicker work, but verify they have done the task properly.
 - Rather than wasting large amounts of context/tokens if able use lesser agents to do the work.
 - Make certain their instruction set is done in a way that they can not mess things up.
 - keep the context for each agent to the lowest amount needed for them to get the job done.
 - if in doubt then you do it.
---
...
| `kick_api_docs.txt` | the kick api in LLM format |
